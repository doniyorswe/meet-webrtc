...
const UserDropdown = () => {
    const [selectUser, setSelectUser] = useState(users[2])

    return (
        <DropdownMenu>
            <DropdownMenuTrigger className='bg-background flex items-center gap-2 rounded-lg px-1 py-1 shadow-xs'>
                <Avatar>
            ...
            <DropdownMenuContent align="end" className='w-56 shadow-xs border-none'>
                {/* <DropdownMenuLabel>Task Assignment</DropdownMenuLabel> */}
                {users.map(user => (
                    <DropdownMenuItem key={user.id} onClick={() => setSelectUser(user)} className='hover:bg-secondary/10!'>
